			...
			for (int y = 0; y < 4; y++) {// quatro rodadas para reiniciar fichas
				...
				for (int j = 0; j < nJogadores; j++) {//rodada jogadores
					...
					while(lerAposta!=1 && lerAposta!=2 && lerAposta!=3 && lerAposta!=4){
						int possuiFichas[] = this.jogadores.get(vez).getPossuiFichas();
						System.out.println("Escolhas uma das seguintes fichas: ");
						for (int i = 0; i < possuiFichas.length; i++) {
							System.out.print(possuiFichas[i]+" ");
						}						
						lerAposta = ler.nextInt();
					}
					//realiza a aposta
					boolean realizouAposta = this.jogadores.get(vez).apostarFicha(lerAposta);
					
					while(realizouAposta==false){					
						System.out.println("Você não possui a ficha "+ lerAposta+":");
						int possuiFichas[] = this.jogadores.get(vez).getPossuiFichas();
						System.out.println("Escolhas uma das seguintes fichas: ");
						for (int i = 0; i < possuiFichas.length; i++) {
							System.out.print(possuiFichas[i]+" ");
						...
						lerAposta = ler.nextInt();
						realizouAposta = this.jogadores.get(vez).apostarFicha(lerAposta);
					}
